...
module ReleaseInputParser =
    let private duplicatePropertyErrors scope (element: JsonElement) =
        ...
        |> Seq.choose (fun (name, count) ->
            if count > 1 then
                ...
                Some
                    $"Input property '{path}' appears more than once. Remove duplicate property names so the input is unambiguous."
            else
                None)
        |> Seq.toList
    ...
    let private parseChange index (element: JsonElement) =
        ...
        if element.ValueKind <> JsonValueKind.Object then
            ...
            match duplicatePropertyErrors $"changes[{index}]" element with
        ...
        let values, errors =
            ...
            |> Array.fold
                (fun (values, errors) result ->
                    ...
                    | Error nextErrors -> values, List.rev nextErrors @ errors)
                ([], [])
    ...
    let parse (json: string) =
        try
            ...
            if root.ValueKind <> JsonValueKind.Object then
                ...
                match duplicatePropertyErrors "" root with
                | errors when not (List.isEmpty errors) -> Error errors
                | _ ->
